...
namespace Devastation.BaseRace
{
    public class BaseRace2
    {
        ...
        private void SavePlayerScore(RaceData.Race rp, SSPlayer ssp, TimeSpan time)
        {
            ...
            if (!File.Exists(pBaseFile)) File.Create(pBaseFile).Close();
            ...
            if (!File.Exists(bBaseFile)) File.Create(bBaseFile).Close();
